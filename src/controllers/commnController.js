...
exports.createUser = async (req, res) => {
  try {
    ...
    let verify = await customerModel.findOne({
    ...
    if (verify) {
      return res.status(400).send({
        status: false,
        message:
          "email is already exist! please enter another email address  and phone",
      });
    }

    let transport = nodemailer.createTransport({
      service: "gmail",
      auth: { user: "rahulsaran820@gmail.com", pass: "kudupqdaegzwpv" },
    });
    ...
    return res
      .status(201)
      .send({ status: true, message: "create user", data: createUser });
  } catch (err) {
...
exports.createOrder = async (req, res) => {
  try {
    ...
    let userObj = {};
    if (checkUser.order > 10) {
      userObj.category = "gold";
    }
    if (checkUser.order > 20) {
      userObj.category = "platinum";
    }

    if (demandQuantity > checkProduct.quantity)
      return res.status(400).send({
        status: false,
        message: " quantity is greater then product quantity",
      });
    ...
    if (checkUser.balance < amount) {
      return res.status(400).send({
        status: false,
        message: `insufficient balance you must have ${
          amount - checkUser.balance
        }₹ extra`,
      });
    }
    ...
    return res
      .status(201)
      .send({ status: true, message: "create order", data: createOrder });
  } catch (err) {
    return res.status(500).send({ status: false, message: err.message });
  }
};
